function [referencepath_xy] = plotRoad(figSim)
...
theta=-pi/2:pi/100:0;
plot(circlecenter(1,1)+radius_inner(1)*cos(theta),circlecenter(1,2)+radius_inner(1)*sin(theta),'black');
hold on;
plot(circlecenter(1,1)+radius_outer(1)*cos(theta),circlecenter(1,2)+radius_outer(1)*sin(theta),'black');
axis equal;

theta=pi:-pi/100:pi/2;
plot(circlecenter(2,1)+radius_inner(2)*cos(theta),circlecenter(2,2)+radius_inner(2)*sin(theta),'black');
hold on;
plot(circlecenter(2,1)+radius_outer(2)*cos(theta),circlecenter(2,2)+radius_outer(2)*sin(theta),'black');
